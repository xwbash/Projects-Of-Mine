...
TournamentClaimRewardPopup* TournamentClaimRewardPopup::setup(int placement, int score, std::vector<RewardData> rewards)
{
    ...
    placementBg->setPositionY(bg->getBoundingBox().getMaxY() + bg->getBoundingBox().size.height * -.01f);
    placementBg->setVisible(false);
    ...
    labelConfig.labelColor = Color3B(255, 195, 41);
    ...
    HBLabel* titleLabelUpper = HBLabel::create("Tournament Ended", FONTS::USE_LOCALIZATION, width * .09f);
    titleLabelUpper->setPositionY(placementBg->getPositionY() + width * .15f);
    titleLabelUpper->setTextColor(Color4B(255, 205, 27, 255));
...
void TournamentClaimRewardPopup::playAnimation()
{
...
void TournamentClaimRewardPopup::onExit()
{
    if(MenuNode::current())
    {
...
std::string TournamentClaimRewardPopup::getPlacementString(int placement)
{
    ...
    return StringUtils::format("%d%s Place", placement, suffixes.at(ord).c_str());
}

std::string TournamentClaimRewardPopup::getIconPath(int placement, bool isWinner)
{
    if(!isWinner)
    {
        return RemoteTournamentManager::getInstance()->getMainMenuIconWithTimerPath();
    }
    
    int index = (int)clampf(placement, 0 , 3);
    return RemoteTournamentManager::getInstance()->getChestPath(index);
}
